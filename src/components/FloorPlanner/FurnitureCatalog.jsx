...
// 'Data Center' value: { 'TypeGroup': { 'Brand': [items] } }
// Other category values: [items] (flat array)
  ...
  return (
    ...
    >
      <div style={{
        ...
        background: item.color, border: '1px solid rgba(0,0,0,0.12)',
      }} />
...
const FurnitureCatalog = () => {
  ...
  // Which type groups are open, e.g. Set { 'Server', 'Generator' }
  const [openTypes, setOpenTypes] = useState(new Set(['Server']));
  // Which brands are open, keyed as 'TypeGroup/Brand'
  ...
  const allItems = useMemo(() => {
    ...
    for (const [cat, catVal] of Object.entries(FURNITURE_CATALOG)) {
      if (isTyped(catVal)) {
        // Data Center: { TypeGroup: { Brand: [items] } }
        for (const [typeGroup, brands] of Object.entries(catVal)) {
          if (isBrands(brands)) {
            ...
            // fallback: brands IS already an array
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input
            ...
            style={{
              ...
              fontSize: 11, background: '#fff', color: '#374151', outline: 'none',
            }}
          ...
          {search && (
            <button
              ...
              style={{
                ...
                fontSize: 11, color: '#94A3B8', padding: 0, lineHeight: 1,
              }}
            >✕</button>
          )}
        </div>
        ...
        {searchResults ? (
          ...
          searchResults.length === 0 ? (
            <div style={{ padding: '24px 12px', textAlign: 'center',
              color: '#94A3B8', fontSize: 11 }}>No assets found</div>
          ) : (
            <div>
              <div style={{ padding: '6px 12px 4px', fontSize: 10, color: '#94A3B8',
                  ...
                  <div key={`${typeGroup}-${brand}-${item.type}-${item.name}`}>
                    {(typeGroup || brand) && (
                      <div style={{ padding: '2px 12px 0 18px', fontSize: 9,
                        color: '#94A3B8', fontWeight: 600, letterSpacing: '0.3px' }}>
          ...
          Object.entries(FURNITURE_CATALOG).map(([cat, catVal]) => {
            ...
            const typed  = isTyped(catVal); // Data Center style
              ...
              <div key={cat}>
                ...
                <button
                  ...
                  style={{
                    ...
                    border: 'none',
                    borderBottom: `1px solid ${isOpen ? `${meta.accent}18` : '#F1F5F9'}`,
                ...
                >
                  ...
                  <span style={{ flex: 1, fontSize: 12,
                    ...
                    color: isOpen ? meta.accent : '#374151' }}>{cat}</span>
                  {meta.tag && (
                    <span style={{
                  ...
                  <span style={{ fontSize: 9, fontWeight: 700,
                    color: isOpen ? meta.accent : '#CBD5E1' }}>{isOpen ? '▲' : '▼'}</span>
                </button>
                ...
                {isOpen && !typed && (
                  <div style={{ background: '#FAFBFC', paddingBottom: 4 }}>
                    {catVal.map((item) => (
                      <ItemRow
                        key={`${item.type}-${item.name}`}
                        item={item}
